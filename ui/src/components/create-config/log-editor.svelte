<script lang="ts">
	...
	function get_display_combat(log: Log) {
		const player_one_name = log.names[player_one_index];
		const player_two_name = log.names[player_two_index];

		if (!invert_combat_direction) {
			return {
				left: player_one_name,
				verb: log.kill ? 'has killed' : 'died to',
		...
		return {
			...
			verb: log.kill ? 'died to' : 'has killed',
	...
	function get_logs_string() {
		return logs
			.map((log) => {
				...
				const remaining_indices = Array.from(
					{ length: log.names.length },
					(_, index) => index
				).filter((i) => i !== player_one_index && i !== player_two_index && i !== guild_index);
				const remaining_names = remaining_indices.map((i) => log.names[i]);
				const characters = ` (${remaining_names.join(',')})`;
				return `[${log.time}] ${combat.left} ${combat.verb} ${combat.right} from ${log.names[guild_index]}${characters}`;
			})
			.join('\n');
	}
...
<div class="flex flex-col gap-2 items-center w-full h-full min-h-0 relative">
	<div class="flex gap-1 items-center justify-center w-full flex-shrink-0">
	...
	<div class="w-full flex gap-2 min-h-0 flex-1">
		<div
		...
		>
			...
			{/if}
			{#key `${logs.length === 0}-${list_height}-${invert_combat_direction}`}
				<VirtualList items={logs} let:item={log}>
					<div class="flex gap-2 group py-1 items-center px-1 justify-center w-full">
		...
		<div class="flex-1 flex flex-col gap-2 text-xs h-full overflow-y-auto" bind:clientHeight={list_height}>
	...
	<div class="flex gap-2 flex-shrink-0">
		<Button class="w-32" on:click={upload} disabled={true}>Upload</Button>
