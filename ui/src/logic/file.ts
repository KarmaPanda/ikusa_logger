...
export async function open_pcap_save_location(path = '.') {
	return await os.showSaveDialog('Save packet capture to file', {
		defaultPath: path,
		filters: [{ name: 'Packet capture', extensions: ['pcap'] }]
	});
}
